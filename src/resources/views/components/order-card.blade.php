<div class="my-10 bg-white rounded-lg border shadow-lg p-5">
    <div class="grid lg:grid-cols-3 items-center pb-2 border-b-2">
        <div>
            <h2 class="font-medium text-xl lg:text-3xl">Commande #{{ $index }}</h2>
            <p class="text-gray-600 text-sm"><span class="font-medium">Référence :</span> {{ $order->reference }}</p>
        </div>
        <p class="lg:text-center lg:text-xl">{{ \Carbon\Carbon::parse($order->created_at)->format('j F Y') }}</p>

        <span class="font-medium text-3xl lg:text-5xl lg:text-right">{{ $order->total_price }} &euro;</span>
    </div>

    @foreach ($order->productPurchased as $product)
        <div class="my-5 grid lg:grid-cols-3 gap-8 items-center text-xl">
            ...
            <span class="lg:text-right">{{ $product->price }} &euro;</span>
        </div>
    @endforeach
</div>
